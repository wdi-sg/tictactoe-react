class Board extends React.Component {
  ...
  checkWin() {
    for (let r = 0; r < 3; r++) {
      ...
      for (let c = 0; c < 3; c++) {
        if (
          (c > 0 && this.state.board[r][c] != firstC) ||
    ...
    for (let c = 0; c < 3; c++) {
      ...
      for (let r = 0; r < 3; r++) {
        if (
          (r > 0 && this.state.board[r][c] != firstR) ||
          this.state.board[r][c] == ""
        ) {
          check = false;
        }
    ...
    for (let i = 0; i < 3; i++) {
      //CHECK FOR Dia L WINS
      if (
        (this.state.board[i][i] != firstDiaL && i > 0) ||
    ...
    for (let i = 0; i < 3; i++) {
      //CHECK FOR Dia L WINS
      if (
        (this.state.board[i][2 - i] != firstDiaR && i > 0) ||
        this.state.board[i][2 - i] == ""
      ) {
        check = false;
      }
    }
  ...
  render() {
    ...
    const board = this.state.board.map((row, rowIndex) => {
      const rows = row.map((col, colIndex) => {
        ...
        return (
          <div className="box" onClick={this.clickHandler} id={boxId}>
            {col}
          </div>
        );
      });
