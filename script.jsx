class Board extends React.Component {
  ...
  victory() {
    ...
    for (let i = 0; i < 3; ++i) {
      let horArray = [this.state.board[0][i], this.state.board[1][i], this.state.board[2][i]]
      if (this.state.board[i].every(this.cross) == true || diagOne.every(this.cross) == true || 
      diagTwo.every(this.cross) == true || horArray.every(this.cross)) {
        this.setState({
          game: false
        })
        console.log('X Wins')
      ...
      diagTwo.every(this.circle) == true || horArray.every(this.circle)) {
        this.setState({
          game: false
        })
        console.log('O Wins')
